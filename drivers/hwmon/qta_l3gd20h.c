 ...
 *	ST Microelectronics L3GD20H module.
 *
 *	According to Quanta's standard, Gyroscope measures are
...
#define L3GD20H_DRV_NAME			"l3gd20h"
...
#define L3GD20H_FIFOCTRL_REG		0x2E
 ...
 * Data registers
 */
#define L3GD20H_XAXIS_LSB			0x28
#define L3GD20H_XAXIS_MSB			0x29
#define L3GD20H_YAXIS_LSB			0x2A
#define L3GD20H_YAXIS_MSB			0x2B
#define L3GD20H_ZAXIS_LSB			0x2C
#define L3GD20H_ZAXIS_MSB			0x2D
#define L3GD20H_TEMP				0x26

/**
 * Interrupt control registers
 */
#define L3GD20H_INT_CFG				0x30
#define L3GD20H_INT_SRC				0x31
#define L3GD20H_INT_THRS_XL			0x32
#define L3GD20H_INT_THRS_XH			0x33
#define L3GD20H_INT_THRS_YL			0x34
#define L3GD20H_INT_THRS_YH			0x35
#define L3GD20H_INT_THRS_ZL			0x36
#define L3GD20H_INT_THRS_ZH			0x37
#define L3GD20H_INT_DURATION		0x38

/**
 * Range modes according to the datasheet
 */
typedef enum {
...
typedef enum {
	 ...
	 * Suspended mode.
	 */
	L3GD20H_SUSPENDED = 0x07,
	/**
	 * Low Power mode.
	 * Currently it is not supported.
	 * Setting to Suspended Mode
	 */
	L3GD20H_LOW_POWER = L3GD20H_SUSPENDED,
} l3gd20h_power_modes;

struct l3gd20h_dev {
	struct i2c_client 					*client;
	struct input_polled_dev				*idev_gyro;
#ifdef CONFIG_ST_L3GD20H_TEMP
	struct input_polled_dev				*idev_temp;
#endif
	struct device 						*dev;
	l3gd20h_range_modes				range_mode;
	l3gd20h_power_modes				power_mode;
	struct mutex						lock;
	struct quanta_motion_sensors_platform_data 	*platform;
};

static unsigned int l3gd20h_get_conversion(struct l3gd20h_dev *sensor)
{

	if(sensor == NULL)
	{
		pr_err("%s: L3GD20H sensor structure is null", __FUNCTION__);
		BUG();
		return -ENODEV;
	}
	...
	switch(sensor->range_mode)
	{
		...
		default:
			 ...
			 */
			pr_err("%s: Invalid range mode for L3GD20H driver!", __FUNCTION__);
			BUG();
			return -EINVAL;
	}
}

static int l3gd20h_read_reg(struct l3gd20h_dev *sensor, u8 reg, u8 *value)
{
	int ret;
...
static int l3gd20h_write_reg(struct l3gd20h_dev *sensor, u8 reg, u8 value)
{
	int ret;
...
static int l3gd20h_read_values(struct l3gd20h_dev *sensor, int *x, int *y, int *z)
{
	int error = 0;
	...
	error |= l3gd20h_read_reg(sensor, L3GD20H_XAXIS_LSB, &x_lsb);
	error |= l3gd20h_read_reg(sensor, L3GD20H_XAXIS_MSB, &x_msb);
	error |= l3gd20h_read_reg(sensor, L3GD20H_YAXIS_LSB, &y_lsb);
	error |= l3gd20h_read_reg(sensor, L3GD20H_YAXIS_MSB, &y_msb);
	error |= l3gd20h_read_reg(sensor, L3GD20H_ZAXIS_LSB, &z_lsb);
	error |= l3gd20h_read_reg(sensor, L3GD20H_ZAXIS_MSB, &z_msb);
...
static int l3gd20h_read_temperature(struct l3gd20h_dev *sensor, int *temp)
{
	u8 temp_data = 0x00;
	int error = 0;

	error = l3gd20h_read_reg(sensor, L3GD20H_TEMP, &temp_data);
...
static int l3gd20h_set_range(struct l3gd20h_dev *sensor, l3gd20h_range_modes mode)
{
	int value = 0x00;
	int error = 0x00;
	...
	mutex_lock(&sensor->lock);
	error = l3gd20h_write_reg(sensor, L3GD20H_CTRL_REG4, value);
	mutex_unlock(&sensor->lock);
...
static int l3gd20h_set_power(struct l3gd20h_dev *sensor, l3gd20h_power_modes mode)
{
	return l3gd20h_write_reg(sensor, L3GD20H_CTRL_REG1, mode);
}

#ifdef CONFIG_ST_L3GD20H_TEMP

static void l3gd20h_dev_poll_temp(struct input_polled_dev *dev)
{
	struct l3gd20h_dev *sensor = input_get_drvdata(dev->input);
	int temp;
	int error = 0;

	if(sensor->power_mode != L3GD20H_NORMAL)
		goto out;

	error = l3gd20h_read_temperature(sensor, &temp);
	if(error)
	{
...
static void l3gd20h_dev_poll_values(struct input_polled_dev *dev)
{
	struct l3gd20h_dev *sensor = input_get_drvdata(dev->input);
	int x, y, z;
	int conversion;
	...
	error = l3gd20h_read_values(sensor, &x, &y, &z);
	if(error)
	{
	...
	conversion = l3gd20h_get_conversion(sensor);
...
static ssize_t l3gd20h_sysfs_set_range_mode(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	l3gd20h_range_modes range_mode;
	int error = 0;
	...
	struct l3gd20h_dev *sensor = ipdev->private;
	...
	case L3GD20H_500DPS:
	case L3GD20H_2000DPS:
		range_mode = (l3gd20h_range_modes) new_range;
		error = l3gd20h_set_range(sensor, range_mode);
		if(error)
		{
...
static ssize_t l3gd20h_sysfs_get_range(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct input_dev *idev = to_input_dev(dev);
	struct input_polled_dev *ipdev = input_get_drvdata(idev);
	struct l3gd20h_dev *sensor = ipdev->private;
...
static DEVICE_ATTR(range_mode, S_IWUSR | S_IRUGO, l3gd20h_sysfs_get_range, l3gd20h_sysfs_set_range_mode);
...
l3gd20h_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	u8 client_id = 0;
	int error = 0;
	struct l3gd20h_dev *this = (struct l3gd20h_dev *) kcalloc(sizeof(struct l3gd20h_dev), 1, GFP_KERNEL);
	if (IS_ERR(this))
	{
	 ...
	 */
	error = l3gd20h_read_reg(this, L3GD20H_CHIP_ID, &client_id);
	if(error)
	{
	...
	if(client_id != L3GD20H_CHIP_ID_VALUE)
	{
		dev_err(&(client->dev), "error: device id 0x%X is not the expected (0x%x)\n", client_id, L3GD20H_CHIP_ID_VALUE);
		error = -ENODEV;
		goto err_no_dev;
	}
	 ...
	 */
	error = 0;
	error |= l3gd20h_set_range(this, L3GD20H_250DPS);
	error |= l3gd20h_set_power(this, L3GD20H_NORMAL);
	error |= l3gd20h_write_reg(this, L3GD20H_INT_CFG, 0x00);
	...
	{
		dev_err(&(client->dev), "error: out of memory for gyroscope input device\n");
		error = -ENOMEM;
	 ...
	 */
	this->idev_gyro->poll = l3gd20h_dev_poll_values;
	...
	if(error)
	{
		dev_err(&(client->dev), "error: cannot register the gyroscope input device\n");
		error = -ENOMEM;
		goto err_register_input_accel;
	}

	error = sysfs_create_group(&this->idev_gyro->input->dev.kobj, &l3gd20h_attr_group);
	if (error)
	{
	 ...
	 * Configure the input device
	 */
	this->idev_temp->poll = l3gd20h_dev_poll_temp;
	...
	err_alloc_input_temp:
		sysfs_remove_group(&this->idev_gyro->input->dev.kobj, &l3gd20h_attr_group);
#endif
	err_register_sys_fs:
...
static int __devexit l3gd20h_remove(struct i2c_client *client)
{
	struct l3gd20h_dev *sensor = i2c_get_clientdata(client);

#ifdef CONFIG_ST_L3GD20H_TEMP
	input_unregister_polled_device(sensor->idev_temp);
	input_free_polled_device(sensor->idev_temp);
#endif
	sysfs_remove_group(&sensor->idev_gyro->input->dev.kobj, &l3gd20h_attr_group);
...
static int l3gd20h_suspend(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct l3gd20h_dev *sensor = i2c_get_clientdata(client);

	int error = l3gd20h_set_power(sensor, L3GD20H_SUSPENDED);
...
static int l3gd20h_resume(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct l3gd20h_dev *sensor = i2c_get_clientdata(client);

	int error = l3gd20h_set_power(sensor, L3GD20H_NORMAL);
...
#else
static int l3gd20h_suspend(struct device *dev) { return 0; }
static int l3gd20h_resume(struct device *dev) { return 0; }
#endif

static const struct dev_pm_ops l3gd20h_pm_ops = {
		.suspend = l3gd20h_suspend,
		.resume = l3gd20h_resume,
};
...
static struct i2c_driver l3gd20h_driver = {
	.driver = {
		.name = L3GD20H_DRV_NAME,
		.owner = THIS_MODULE,
		.pm = &l3gd20h_pm_ops,
	},
	.probe = l3gd20h_probe,
	.remove = __devexit_p(l3gd20h_remove),
	.id_table = l3gd20h_id,
};
...
l3gd20h_init(void)
{
	...
	res = i2c_add_driver(&l3gd20h_driver);
	if (res < 0)
	{
		pr_err("%s: add l3gd20h i2c driver failed\n", __FUNCTION__);
		return -ENODEV;
	}
	return res;
}
...
l3gd20h_exit(void)
{
	i2c_del_driver(&l3gd20h_driver);
}

MODULE_AUTHOR("Mauricio Cirelli <mauricio.cirelli@quantatec.com.br>");
MODULE_DESCRIPTION("ST L3GD20H 3-Axis Gyroscope Sensor driver");
MODULE_LICENSE("GPL");

module_init(l3gd20h_init);
module_exit(l3gd20h_exit);
