...
#define ADV7180_DEFAULT_C              0x0D     /* Default Value C */

#define ADV7180_TIMEPERFRAME           30
...
static inline int adv7180_read(u8 reg)
{
	...
	if (val < 0) {
		dev_err(&adv7180_data.i2c_client->dev, "read reg error: reg=%2x \n", reg);
	}
	return val;
}
...
{
	int ret = i2c_smbus_write_byte_data(adv7180_data.i2c_client, reg, val);
	if (ret < 0) {
		dev_err(&adv7180_data.i2c_client->dev, "write reg error:reg=%2x,val=%2x\n", reg, val);
		return ret;
	}
	return 0;
}
...
static int adv7180_set_input_source(enum v4l2_adv7180_inputs_t input)
{
	...
	{
		dev_err(&adv7180_data.i2c_client->dev, "selecting input source is only available in cvbs mode!");
		return -EPERM;
	}
	...
	if(ret) {
		dev_err(&adv7180_data.i2c_client->dev, "error while selecting input source: %d", ret);
		return ret;
	}

	dev_info(&adv7180_data.i2c_client->dev, "selected input source %d", input);
...
{
	if (fmt->index >= ARRAY_SIZE(video_fmts))
		return -EINVAL;

	fmt->pixelformat = adv7180_data.pix.pixelformat;
			 ...
			 const struct i2c_device_id *id)
{
	...
	dev_dbg(&adv7180_data.i2c_client->dev, "%s:adv7180 probe i2c address is 0x%02X \n", __func__, adv7180_data.i2c_client->addr);

	/*! Read the revision ID of the tvin chip */
	rev_id = adv7180_read(ADV7180_IDENT);
	if(rev_id < 0)
	{
		dev_err(&adv7180_data.i2c_client->dev, "device not found %d \n", rev_id);
		return -ENODEV;
	}

	dev_info(&adv7180_data.i2c_client->dev, "Analog Device adv7%2X0 detected! \n", rev_id);

	/*! ADV7180 initialization. */
	adv7180_hard_reset(tvin_plat->cvbs);
	adv7180_data.cvbs = tvin_plat->cvbs;
	if(adv7180_data.cvbs)
		adv7180_set_input_source(ADV7180_AIN1);
