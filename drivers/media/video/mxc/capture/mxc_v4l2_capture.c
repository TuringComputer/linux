...
static int mxc_v4l2_s_param(cam_data *cam, struct v4l2_streamparm *parm)
{
	...
	if ((cam->device_type != 1) && (parm->parm.capture.capturemode == currentparm.parm.capture.capturemode)) {
		return 0;
	}
	...
	if (ifparm.if_type == V4L2_IF_TYPE_BT656) {
		...
		pr_debug("   bt656.clock_curr=mclk=%d\n", ifparm.u.bt656.clock_curr);
		if (ifparm.u.bt656.clock_curr == 0) {
			csi_param.clk_mode = IPU_CSI_CLK_MODE_CCIR656_INTERLACED;
			pr_debug("mxc_v4l2_s_param: csi_param.clk_mode = IPU_CSI_CLK_MODE_CCIR656_INTERLACED.\n");
		} else {
	...
	} else if (ifparm.if_type == V4L2_IF_TYPE_BT1120) {
		pr_debug("   bt1120.clock_curr=mclk=%d\n", ifparm.u.bt1120.clock_curr);
		if (ifparm.u.bt1120.clock_curr == 0) {
			csi_param.clk_mode = IPU_CSI_CLK_MODE_CCIR1120_INTERLACED_SDR;
			pr_debug("mxc_v4l2_s_param: csi_param.clk_mode = IPU_CSI_CLK_MODE_CCIR1120_INTERLACED.\n");
		} else {
			if (cam->device_type != 1)
				csi_param.clk_mode = IPU_CSI_CLK_MODE_GATED_CLK;
			else
...
static int mxc_v4l_open(struct file *file)
{
	...
	if (cam->open_count++ == 0) {
		...
		if (ifparm.if_type == V4L2_IF_TYPE_BT656) {
			...
			csi_param.mclk = ifparm.u.bt1120.clock_curr;
			csi_param.pixclk_pol = ifparm.u.bt1120.latch_clk_inv;

			if (ifparm.u.bt1120.mode
					== V4L2_IF_TYPE_BT1120_MODE_BT_8BIT)
				csi_param.data_width = IPU_CSI_DATA_WIDTH_8;
			else if (ifparm.u.bt1120.mode
					== V4L2_IF_TYPE_BT1120_MODE_BT_16BIT)
				csi_param.data_width = IPU_CSI_DATA_WIDTH_16;
...
static void init_camera_struct(cam_data *cam, struct platform_device *pdev)
{
	...
	*(cam->video_dev) = mxc_v4l_template;
	...
	/* Need to determine how to set this correctly with multiple video input devices. */
#ifdef CAPTURE_ON_CSI1
	cam->csi = 1;
#else
	cam->csi = 0;
