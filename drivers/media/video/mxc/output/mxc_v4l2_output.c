...
static unsigned int ipu_vdi_in_max_width_size;
static unsigned int ipu_vdi_in_max_height_size;
static unsigned int ipu_vdi_out_max_width_size;
static unsigned int ipu_vdi_out_max_height_size;
static int vdi_delay = 0;
...
static void mxc_v4l2out_timer_handler(unsigned long arg)
{
	...
	if (INTERLACED_CONTENT(vout) && (!vout->pp_split) && 
...
{
	u16 out_stride; /* stride of output channel */
	u32 eba_offset;
	u16 x_pos;
	u16 y_pos;
	...
	if (vout->cur_disp_output == 5) {
		...
		eba_offset = (vout->xres*y_pos + x_pos) *
				bytes_per_pixel(params.mem_prp_vf_mem.out_pixel_fmt);
		eba_offset = 0;
	}

	vout->bpp = fmt_to_bpp(params.mem_prp_vf_mem.out_pixel_fmt);
	out_stride = vout->xres * bytes_per_pixel(params.mem_prp_vf_mem.out_pixel_fmt);

	/* split VDI by two stripes, the by pass is impossible*/
	if (vout->pp_split) {
		...
		if (vout->pp_split != 3) {
			ipu_calc_stripes_sizes(
				params.mem_prp_vf_mem.in_width, /* input frame width;>1 */
				params.mem_prp_vf_mem.out_width, /* output frame width; >1 */
				((params.mem_prp_vf_mem.out_width >> 1) & (~(0x7))) + 8, /* half and align in 8 pixel */
			...
			vout->pp_left_stripe.output_column = vout->pp_left_stripe.output_column *
								fmt_to_bpp(params.mem_prp_vf_mem.out_pixel_fmt) / 8;
		...
		/* updare parameters */
		params.mem_prp_vf_mem.in_width = vout->pp_left_stripe.input_width;
		params.mem_prp_vf_mem.out_width = vout->pp_left_stripe.output_width;
		out_width = vout->pp_left_stripe.output_width;
		/* for using in vdi_init*/
		params.mem_prp_vf_mem.outh_resize_ratio = vout->pp_left_stripe.irr;
		}
		if (vout->pp_split != 2) {
			ipu_calc_stripes_sizes(
				params.mem_prp_vf_mem.in_height, /* input frame width;>1 */
				params.mem_prp_vf_mem.out_height, /* output frame width; >1 */
				((params.mem_prp_vf_mem.out_height >> 1) & (~(0x7))) + 8, /* half and align in 8 pixel */
				(((unsigned long long)1) << 32),/* 32bit for fractional */
				1,	/* equal stripes */
				params.mem_prp_vf_mem.in_pixel_fmt,
				params.mem_prp_vf_mem.out_pixel_fmt,
			...
			params.mem_prp_vf_mem.outv_resize_ratio = vout->pp_up_stripe.irr;
			params.mem_prp_vf_mem.in_height = vout->pp_up_stripe.input_width;/*height*/
	...
	if (init_VDI_in_channel_buffer(vout,
				       ...
				       vout->v2f.fmt.pix.bytesperline / bytes_per_pixel(params.mem_prp_vf_mem.in_pixel_fmt),
				       vout->offset.u_offset,
...
static int mxc_v4l2out_streamon(vout_data *vout)
{
	...
	in_width = vout->v2f.fmt.pix.width;
	in_height = vout->v2f.fmt.pix.height;
	vdi_delay = 0;
	if (INTERLACED_CONTENT(vout)) {
		...
		if ((in_width > ipu_vdi_in_max_width_size) || 
			(in_height > ipu_vdi_in_max_height_size) || 
			(out_width > ipu_vdi_out_max_width_size) ||
			(out_height > ipu_vdi_out_max_height_size))
		{
			vdi_split = 4;

			if((in_height) < 720 && (out_height > ipu_vdi_out_max_height_size))
				vdi_delay = 1;
		}
	}
	...
	if (!INTERLACED_CONTENT(vout)) {
		...
		if (vdi_split) {
			...
			vout->ipu_buf[1] = vout->ipu_buf[0];
			vout->frame_count = 1;
			if (((in_width > ipu_vdi_in_max_width_size) || (out_width > ipu_vdi_out_max_width_size)) && 
				((in_height > ipu_vdi_in_max_height_size) || (out_height > ipu_vdi_out_max_height_size)))
				vdi_split = 1; /*4 stripes*/
			else if ((in_width > ipu_vdi_in_max_width_size) || (out_width > ipu_vdi_out_max_width_size))
				vdi_split = 2; /*two horizontal stripes */
			else
				vdi_split = 3; /*2 vertical stripes*/
		} else {
			vout->ipu_buf[1] = -1;
			vout->frame_count = 1;
		}
		vout->pp_split = vdi_split;
	} else {
