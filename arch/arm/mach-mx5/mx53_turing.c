...
static struct usb_mass_storage_platform_data mass_storage_data = {
	.nluns		= 3,
	.vendor		= "Turing",
	.product	= "MX53 Turing Android Phone",
	.release	= 0x0100,
};
...
static void __init
fixup_android_board(struct machine_desc *desc, struct tag *tags, char **cmdline, struct meminfo *mi)
{
	char *str;
	struct tag *t;
...
mx53_turing_io_init(void)
{
	...
	/* HDMI CEC_D */
	gpio_request(GPIO22, "HDMI_CEC_D");
	gpio_direction_output(GPIO22, 1);
}
 ...
 * Freescale is important here, because Android will use it as the "hardware manufacturer" for init.rc and HAL.
 */
MACHINE_START(MX53_TURING, "Freescale MX53 TURING Board")
